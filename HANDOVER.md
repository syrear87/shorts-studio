# 인수인계 — 클라우드 세션 → 로컬 Claude Code (2026-07-29 오전)

## 0. 지금 당장 (2026-07-29 밤 갱신 — 과거 항목은 전부 해결됨)
1. **평시 운영**: 슬롯은 launchd가 자동 기동(평일 07/11/16/19 · 주말 08/11/14/17/20). 수동 재실행은 슬롯이 실제로 죽었을 때만 `/usr/bin/python3 bin/daily_runner.py` — **실행 전 logs/daily-오늘*.log와 텔레그램으로 이미 성공했는지 반드시 확인(중복 게시 방지)**.
2. YouTube API 감사: **제출 완료, 심사 대기** — syrear87@gmail.com 회신 감시. 통과 시 config.json `upload_mode: api_public`.
3. 인스타 릴스 자동화: 코드 완성, 디렉터의 Meta 토큰 투입 대기(keys.env에 IG_ACCESS_TOKEN/IG_USER_ID → config.json `instagram: on`).
4. 매일 22:30 워치독 하트비트가 텔레그램으로 와야 정상 — 안 오면 스케줄러/채널 장애.

## 1. 프로젝트 연혁 (요약)
- **07-28**: 게임 스튜디오 프로젝트(~/Dev/indie-ios-game) 폐기 → 숏츠 스튜디오로 전환. 스캐폴드 구축, OAuth·Pexels 셋업, 1편 제작("반도체 치킨게임" — 코스피 -10.84% 폭락일 소재). 디렉터 피드백 3건(배너 찌그러짐/배경 미스매치/결론 밋밋) → 렌더러 수정 완료.
- **07-29**: 2편 제작("타조 효과" — 폭락 이튿날 심리). 디렉터가 Shorts+릴스에 2편 게시. 하루 3회 체제(7:30/11:00/18:00) + 성우 자동 선택 도입. 브랜드 세트 제작(`assets/brand/`). 채널명 "1일 1지식", 핸들 추천: **@1day1know** (플랜B: @1day1know_kr → @harujisik).

## 2. 사고 이력과 교훈 (같은 함정에 빠지지 말 것)
| 사고 | 원인 | 수정 |
|---|---|---|
| 07-29 07:30 즉사 | launchd 최소 PATH → claude(node 스크립트)가 node를 못 찾음 | daily_runner가 `zsh -l -c` 로그인 셸 경유로 실행 (적용됨) |
| 07-29 11:00 즉사 | claude CLI OAuth 만료 | 사용자 재로그인 완료 (13시 pong 실측, 해소) |
| 07-29 11:00 '조용한 죽음' | `claude -p`는 인증 실패에도 **종료코드 0** → 러너가 경보를 안 보냄 | daily_runner가 세션 후 로그 내용 검사(인증 오류 문구·200자 미만)로 경보 (적용됨) |
| 무인 세션 권한 정지 | `.claude/settings.json` allow에 없는 명령 실행 시 프롬프트에 걸려 세션이 영원히 멈춤 (전 게임 프로젝트를 3회 죽인 패턴) | allow 목록 정비됨. **새 명령 추가 시 allow에 먼저 등록할 것. 복합 셸 명령(&&·파이프) 금지** |
| 배경 영상 미스매치(재봉틀 사고) | Pexels 세로 전용 검색 → 니치 소재는 엉뚱한 영상 | bg_query 배열 다중검색 + 가로 HD 폴백 (적용됨) |
| 대본 58초 초과 | 발화 길이 미통제 | 90~110단어 강제 + QA 길이 검사 (DAILY_PROMPT) |
| (참고) 원격 세션의 git index.lock 잔류 | 원격 마운트는 파일 삭제 불가 | 로컬에서는 발생 안 함. `_to_delete/` 폴더는 원격 세션 찌꺼기 — 통째로 삭제해도 됨 |

## 3. 현재 상태
- 게시: 2편 (치킨게임 52초·타조 52초, 둘 다 Shorts+릴스). `out/`에 원본.
- 스케줄: launchd 평일4·주말5 + 22:30 워치독 등록. 온디맨드 러너는 2026-07-29 감사에서 임의코드 실행 통로로 판정되어 **폐지됨** (스케줄 변경은 plist 수정 후 사람이 launchctl unload/load 두 줄).
- 텔레그램: 봇 발신 정상(영상 파일+메타 자동 발송). 게임 repo의 수신 브리지(tg_bridge)는 구코드로 상주 중 — 재시작되면 수정본 적용되지만 현재 용도 없음.
- 성과 수집: 채널에 영상이 쌓이면 `pipeline/fetch_analytics.py` — 아직 미가동. DAILY_PROMPT가 analytics를 읽어 기획에 반영하는 루프는 데이터 생기면 자동 작동.

## 4. YouTube API 감사 — **제출 완료 (2026-07-29 오후), 심사 대기**
- 폼 제출 완료("이메일이 발송됨" 확인 화면). 상세는 DECISIONS.md S-002 참조.
- **다음 액션**: syrear87@gmail.com으로 오는 구글 회신 감시(수일~수주). 추가 자료 요구가 올 수 있음 — 증빙 원본은 `~/Desktop/shorts-studio-audit/`에 보관됨.
- **통과 시**: `config.json`의 `upload_mode`를 `phase0_telegram` → `api_public`으로 변경하면 텔레그램 수동 업로드가 사라지고 완전 자동 전환.
- 저장소는 public으로 push됨(github.com/syrear87/shorts-studio, 시크릿 무검출 검증 완료). 이후 커밋도 push 가능(민감정보만 주의).

## 5. 디렉터(사용자) 성향 메모
- 터미널 명령 부탁을 싫어함 — 가능한 한 직접 실행하고, 부탁할 땐 복붙 한 줄로.
- 품질 피드백이 구체적이고 정확함(배너 픽셀 결함까지 잡아냄). 피드백은 즉시 파이프라인 규칙으로 영구 반영할 것.
- 소재 취향: "오늘 사람들이 실제로 이야기하는 것"에서 출발하는 지식. 밋밋한 결론 싫어함.
- 보고는 간결하게, 결과 중심으로.

## 사고 추가 (2026-07-30)
| 사고 | 원인 | 수정 |
|---|---|---|
| 07-30 17:00 슬롯 즉사 | Anthropic API 529 Overloaded — 일시적 과부하인데 러너에 재시도가 없어 슬롯 하나가 통째로 증발 | daily_runner에 일시장애 재시도 추가(3분→7분, 최대 2회). 마커: 529/overloaded/429/rate_limit/연결오류/500. **로그 800자 초과 시 재시도 금지**(세션이 실제 작업했을 수 있어 중복 게시 방지). 실제 17시 로그로 판정 검증 완료 |
| 07-30 점심 3연속 기각 | 자막+스톡배경 포맷은 화면이 설명을 못 돕는데, '설명이 필요한 개념' 소재를 계속 선택(펨토셀 4단계 인과 / 그래미 제도+역사 병렬) | DAILY_PROMPT에 **포맷 적합성 게이트** 신설 — 훅 문장 자체가 결론인 1단계 소재만 채택, 커뮤니티 화력 1위여도 버릴 것 |
| 07-30 20:00 기각 소재 재제작 | 세션이 기각 소재를 **"채택 안 됨 = 재도전 가능"**으로 해석(리포트에 명시). 근본 원인 2개: ①"기각≠미사용" 규칙 부재 — 중복 금지 규칙은 '이미 쓴 소재'만 막았다 ②topics_used.md에 같은 소재가 '게시본'과 '기각'으로 동시 기재된 기록 오류(클라우드 세션이 발송 시점에 '게시본'으로 적고 기각 후 줄만 추가함) | `content/REJECTED.md`(영구 금지 목록) 신설 + DAILY_PROMPT 최우선 규칙으로 "기각은 차단, 재도전 추론 자체가 금지, 기획자 5명 전원에게 목록 전달" 추가. topics_used.md는 **상태 태그([게시]/[영구금지]/[보류]/[미가동]) 의무 + 중복 기재 금지 + 상태 변경 시 줄 수정** 규칙 도입 |
| 08-01 14:00 발송 실패 | 배경 영상이 고디테일이라 CRF 20 인코딩이 12.75Mbps로 튀어 49초 영상이 **78MB** — 텔레그램 봇 한도(50MB) 초과로 영상 대신 파일 경로만 전달됨 | make_short.py 인코딩에 **`-maxrate 6M -bufsize 12M` 상한 + CRF 20→22**, 그래도 45MB 초과 시 **CRF 26→30으로 자동 재인코딩** 루프 추가. 회귀 검증: 기존 편 재렌더 19.9MB(화질 이상 없음), 사고 편 78→26.5MB |
| 08-01 설명 빈약·해시태그 누락 | 손으로 만든 편에만 있던 설명 형식(요약→본문→불릿→출처→해시태그)을 **규칙에 안 적어놔서** 자동 세션은 규격 없이 99자로 작성. 전수 조사 결과 **자동 세션 게시본 13건 전부 미달, 2건은 해시태그 0개** | DAILY_PROMPT에 설명 규격 신설(최소 300자·불릿·출처·해시태그 5개 필수, '영상에 못 넣은 정보를 채우는 자리'). upload_youtube에 `check_meta()` 게이트 추가 — 규격 미달 시 게시 차단 + 텔레그램 경보 |
