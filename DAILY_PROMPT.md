...
5. `content/오늘날짜.json`(대본, sample_honey.json 형식 — 소재에 맞는 영어 배경영상 검색어를 `bg_query`로 반드시 포함)과 `content/오늘날짜.meta.json`(title/description/tags) 저장.
6. 렌더: `.venv/bin/python3 pipeline/make_short.py content/오늘날짜.json` 실행 (한 줄 단순 명령).
7. QA-사후: ffmpeg로 프레임 3장을 추출해 Read로 직접 보고(자막 잘림·깨짐), ffprobe로 길이 20~50초 확인. 불합격이면 게시 중단+보고.
8. 게시: `.venv/bin/python3 pipeline/upload_youtube.py out/오늘날짜.mp4 content/오늘날짜.meta.json`.
